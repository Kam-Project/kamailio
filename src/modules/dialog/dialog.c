 ...
 */
static inline void internal_rpc_dump_dlg(dlg_cell_t *dlg, FILE* dialogf)
{
 ...
 * \param with_context if 1 then the dialog context will be also printed
 */
static void internal_rpc_dump_dlgs(rpc_t *rpc, void *c, int with_context)
{
	dlg_cell_t *dlg;
	...
	for( i=0 ; i<d_table->size ; i++ ) {
		...
		for( dlg=d_table->entries[i].first ; dlg ; dlg=dlg->next ) {
			internal_rpc_dump_dlg(dlg, dialogf);
		}
...
static const char *rpc_dump_dlgs_doc[2] = {
...
static void rpc_dump_dlgs(rpc_t *rpc, void *c) {
	internal_rpc_dump_dlgs(rpc, c, 0);
}
...
static rpc_export_t rpc_methods[] = {
	...
	{"dlg.dump", rpc_dump_dlgs, rpc_dump_dlgs_doc, 0},
